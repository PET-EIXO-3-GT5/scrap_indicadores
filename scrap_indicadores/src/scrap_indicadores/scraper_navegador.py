...
async def main():
    """
    Demonstration main function. Scrapes multiple DATASUS Tabnet tables
    based on the requirements defined in bases.md for Piauí (PI),
    focusing on Picos (IBGE code 220800).
    """
    ...
    print("\n=======================================================")
    print("INICIANDO DOWNLOAD DE BASES DATASUS COM PLAYWRIGHT SCRAPER")
    print("=======================================================")
    
    # 1. SIM Geral (Mortalidade Geral 2024)
    print("\n--- [SIM] Mortalidade Geral ---")
    sim_csv_path = os.path.join(data_dir, "sim_geral_2024.csv")
    try:
        await scraper.download_csv(
            url="http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sim/cnv/obt10pi.def",
            output_path=sim_csv_path,
            linha="Município",
            coluna="--Não-Ativa--",
            incremento="Óbitos_p/Residênc",
            periodos="2024"
        )
        df_sim = parse_tabnet_csv(sim_csv_path)
        picos_sim = df_sim[df_sim["cod_ibge"] == picos_code]
        print(f"Resultados SIM Geral 2024 para Picos:\n{picos_sim.to_string(index=False)}")
    except Exception as e:
        print(f"Erro no download/parse do SIM Geral: {e}")

    # 2. SIHSUS Geral (Morbidade Hospitalar / Internações 2024)
    print("\n--- [SIHSUS] Internações (Morbidade Hospitalar 2024) ---")
    sih_csv_path = os.path.join(data_dir, "sih_geral_2024.csv")
    try:
        await scraper.download_csv(
            url="http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sih/cnv/nrpi.def",
            output_path=sih_csv_path,
            linha="Município",
            coluna="--Não-Ativa--",
            incremento="Internações",
            periodos="2024"  # Selects all matching months
        )
        df_sih = parse_tabnet_csv(sih_csv_path)
        picos_sih = df_sih[df_sih["cod_ibge"] == picos_code]
        print(f"Resultados SIHSUS 2024 para Picos:\n{picos_sih.to_string(index=False)}")
    except Exception as e:
        print(f"Erro no download/parse do SIHSUS: {e}")

    # 3. SINAN Dengue (Dengue 2024)
    print("\n--- [SINAN] Casos de Dengue ---")
    sinan_csv_path = os.path.join(data_dir, "sinan_dengue_2024.csv")
    try:
        await scraper.download_csv(
            url="http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sinannet/cnv/denguebpi.def",
            output_path=sinan_csv_path,
            linha="Município de notificação",  # Dengue uses this row label
            coluna="--Não-Ativa--",
            incremento="Casos_Prováveis",
            periodos="2024"
        )
        df_sinan = parse_tabnet_csv(sinan_csv_path)
        picos_sinan = df_sinan[df_sinan["cod_ibge"] == picos_code]
        print(f"Resultados SINAN Dengue 2024 para Picos:\n{picos_sinan.to_string(index=False)}")
    except Exception as e:
        print(f"Erro no download/parse do SINAN: {e}")

    # 4. SIS-PNI (Vacinações / Doses 2022)
    print("\n--- [PNI] Imunizações (Doses Aplicadas 2022) ---")
    pni_csv_path = os.path.join(data_dir, "pni_doses_2022.csv")
    try:
        await scraper.download_csv(
            url="http://tabnet.datasus.gov.br/cgi/dhdat.exe?bd_pni/dpnibr.def",
            output_path=pni_csv_path,
            linha="Município",
            coluna="--Não-Ativa--",
            incremento="Doses_aplicadas|QT_DOSE",
            periodos="2022",
            filtros={"SUnidade da Federação": "Piauí"}  # Select Piauí UF
        )
        df_pni = parse_tabnet_csv(pni_csv_path)
        picos_pni = df_pni[df_pni["cod_ibge"] == picos_code]
